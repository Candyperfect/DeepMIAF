...
def main(args):
    ...
    # load model config 
    with open('./DeepMIAF.yaml', 'r', encoding='UTF-8') as f:
    ...
    model = DeepMIAF(seq_feature_matrix.shape[1],text_feature_matrix.shape[1],struct_feature_matrix.shape[1], model_config['n_hidden'], label_matrix.shape[1],
    ...
    model.load_state_dict(torch.load(os.path.join(args.model_dir, F'DeepMIAF_{args.ontology}_{args.model_id}.ckp'))) 
    pred_x_score_list, pred_y_score_list = model.inference(g, pred_index, seq_feature_matrix, text_feature_matrix,struct_feature_matrix, label_matrix, model_config['batch_size'], device)
    ...
    if args.input_file == None:
        with open(os.path.join(result_dir, F'{ont}_prediction_DeepMIAF_{args.model_id}.txt'), 'w') as f:
        ...
        with open(os.path.join(result_dir, args.input_file+f'_DeepMIAF_prediction.txt'), 'w') as f:
            for i in range(pred_combine_score.shape[0]):
                for j in range(pred_combine_score.shape[1]):
                    f.write(F'{tmp_pid_list[i]}\t{label_classes[j]}\t{pred_combine_score[i, j]}\n')
...
if __name__ == '__main__':
    ...
    logger.info("Running the DeepMIAF model for prediction.")
