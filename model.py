...
class DeepMIAF(nn.Module): 
    def __init__(self, in_seqdim, in_textdim, in_strdim, h_dim, out_dim, n_hidden_layer, n_prop_step, mlp_drop=0.0,
                 attn_heads=1, feat_drop=0.0, attn_drop=0.0, residual=True, share_weight=True):
        super(DeepMIAF, self).__init__()
        self.n_prop_step = n_prop_step
        self.dropout = mlp_drop
        self.proj1 = nn.Linear(in_seqdim, h_dim)
        self.proj2 = nn.Linear(in_textdim, h_dim)
        self.proj3 = nn.Linear(in_strdim, h_dim)
        
        self.gate12 = nn.Sequential(
            nn.Linear(2 * h_dim, h_dim),
            nn.Sigmoid()  # 输出范围[0,1]
        )
        self.gate13 = nn.Sequential(
            nn.Linear(2 * h_dim, h_dim),
            nn.Sigmoid()
        )
        self.gate23 = nn.Sequential(
            nn.Linear(2 * h_dim, h_dim),
            nn.Sigmoid()
        )

        self.embed_bias1 = nn.Parameter(torch.zeros(h_dim))
        self.embed_bias2 = nn.Parameter(torch.zeros(h_dim))
        self.embed_bias3 = nn.Parameter(torch.zeros(h_dim))
    ...
    def forward(self, blocks, x_seq, x_text, x_struct, y=None):
        v1_proj = F.relu(self.proj1(x_seq)+ self.embed_bias1)
        v2_proj = F.relu(self.proj2(x_text)+ self.embed_bias2)
        v3_proj = F.relu(self.proj3(x_struct)+ self.embed_bias3)
        
        # 2. 交叉生成门控
        g12 = self.gate12(torch.cat([v2_proj, v3_proj], dim=-1))  # 用v2和v3生成v1的门控
        g13 = self.gate13(torch.cat([v1_proj, v3_proj], dim=-1))  # 用v1和v3生成v2的门控
        g23 = self.gate23(torch.cat([v1_proj, v2_proj], dim=-1))
        
        h = (g12 * v1_proj) + (g13 * v2_proj) + (g23 * v3_proj)   
        h = self.mlp(h)
    ...
    def inference(self, g, idx, x_seq, x_text, x_struct, y, batch_size, device): 
        self.eval()
        ...
        with dataloader.enable_cpu_affinity():
            for input_nodes, _, blocks in dataloader:
                ...
                batch_text = torch.from_numpy(x_text[input_nodes.numpy()]).float().to(device)
                ...
                batch_x_hat, batch_y_hat = self.forward(blocks, batch_seq, batch_text, batch_struct, batch_y)
