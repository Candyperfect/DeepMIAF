...
def main(args):
    ...
    logger.info(F'Evaluate DeepMIAF model.')
    ...
    pred_matrix = get_pred_matrix(os.path.join(result_dir, F'{ont}_prediction_DeepMIAF_{args.model_id}.txt'), pid2index, go2index)
    ...
    with open('/root/autodl-tmp/DeepMIAF-master/result/log_d.txt', 'a') as f:  # 'a'表示追加模式
        f.write(F'{ont}_prediction_DeepMIAF_{args.model_id}\n')
        f.write("Aspect: {} | Fmax: {:.4f} | Smin: {:.4f} | AUPR: {:.4f} | threshold: {:.2f}\n".format(ont, fmax_, smin_, aupr_, threshold))
