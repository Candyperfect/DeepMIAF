...
def train(args, dataset):
    ...
    # dataset
    g, seq_feature_matrix, text_feature_matrix, struct_feature_matrix, label_matrix, train_idx = dataset['g'], dataset['seq_feature'], dataset['text_feature'],dataset['struct_feature'], dataset['label'], dataset['train_idx'].astype(np.int32)
    ...
    model = DeepMIAF(seq_feature_matrix.shape[1], text_feature_matrix.shape[1], struct_feature_matrix.shape[1], args.n_hidden, label_matrix.shape[1],
    ...
    for epoch in range(args.n_epochs):
        ...
        with train_dataloader.enable_cpu_affinity():
            for i, (input_nodes, seeds, blocks) in enumerate(train_dataloader):
                ...
                batch_text = torch.from_numpy(text_feature_matrix[input_nodes.numpy()]).float().to(device)
                ...
                logits, _ = model(blocks, batch_seq, batch_text, batch_struct)
        ...
        for input_nodes, _, blocks in valid_dataloader:
            ...
            batch_text = torch.from_numpy(text_feature_matrix[input_nodes.numpy()]).float().to(device)
            batch_struct = torch.from_numpy(struct_feature_matrix[input_nodes.numpy()]).float().to(device)
            batch_pred, _ = model(blocks, batch_seq, batch_text, batch_struct)
        ...
        if fmax_ > best_fmax:
            ...
            best_fmax = fmax_
            torch.save(model.state_dict(), os.path.join(args.model_dir, F"DeepMIAF_{args.ontology}_{args.model_id}.ckp"))
...
def main(args):
    ...
    text_feature_matrix = np.load(f'{data_dir}/text_feature.npy')
    logger.info(F'Shape of seq feature matrix: {seq_feature_matrix.shape}')
    logger.info(F'Shape of text feature matrix: {text_feature_matrix.shape}')
    ...
    dataset = dict()
    dataset['g'], dataset['seq_feature'], dataset['text_feature'], dataset['struct_feature'], dataset['label'], dataset['train_idx'] = g, seq_feature_matrix, text_feature_matrix, struct_feature_matrix, label_matrix, train_idx
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='train DeepMIAF model.')
    ...
    parser.add_argument("--lr", type=float, default=1e-3,
                        help="learning rate")
    parser.add_argument("--n_hidden", type=int, default=768,
                        help="hidden size")
    ...
    parser.add_argument("--model_dir", type=str, default='./DeepMIAF_models',
                        help='path for save the model parameters')
    
    args = parser.parse_args()

    logger.info("Running the training script for DeepMIAF model.")
